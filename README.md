...
**Демо-доступ:** `manager@atlas.com` / `123456` — руководитель · `master@atlas.com` / `123456` — мастер · `elena.senior@atlas.com` / `123456` — старший мастер · `maxim.petrov@atlas.com` / `123456` — мастер (Красноярск) · `admin@atlas.com` / `123456` — администратор

**Что внутри:** 6 миксов с ингредиентами, лайками и комментариями · 5 постоянных клиентов с предпочтениями · 4 категории и 5 статей базы знаний · 3 официальные рецептуры · уведомления от руководителя · акт инвентаризации по категориям табака (А/C/D).
...
Интерфейс и права меняются в зависимости от роли: у каждой из четырёх ролей своё меню, свой набор API-эндпоинтов (проверка роли на сервере, в каждом обработчике) и свой дашборд.

| Возможность | 🎨 Мастер | ⭐ Старший | 🧭 Админ | 🛡 Руководитель |
|---|:---:|:---:|:---:|:---:|
| Личные и публичные миксы, лайки, комментарии | ✅ | ✅ | — | только лента |
| CRM постоянников: база + личные/общие заметки | ✅ | ✅ | ✅ | ✅ |
| Инвентаризация: собственные сессии пересчёта | ✅ | ✅ | — | — |
| Инвентаризация: все сессии команды + справочник тары | — | ✅ | — | ✅ |
| База знаний: статьи `COMMON` + `MASTER` | ✅ | ✅ | — | все |
| База знаний: статьи `ADMIN` | — | — | ✅ | все |
| База знаний: создание/редактирование, категории, «официальные» статьи | — | — | — | ✅ (+ автор правит своё) |
| Рецептуры: чтение | ✅ | ✅ | ✅ | ✅ |
| Рецептуры: создание/редактирование/удаление | — | — | — | ✅ |
| Список сотрудников (просмотр) | — | ✅ | ✅ | ✅ |
| Управление: подтверждение, роли, создание/удаление аккаунтов | — | — | — | ✅ |
| Рассылка уведомлений команде | — | — | — | ✅ |
| Центр безопасности: журнал событий, бэкапы | — | — | — | ✅ |

Зачем так:

- **Мастер** — рабочий инструмент: свои миксы и заметки о клиентах, пересчёт табака на смене. Чужие данные и настройки ему не нужны.
- **Старший мастер** — то же + контроль смены: видит сессии инвентаризации всей команды и ведёт справочник тары.
- **Админ** — офисная роль без «кальянной» специфики: база знаний уровня администрации и CRM, без миксов и инвентаризации.
- **Руководитель** — полный контур управления: кадры (с подтверждением и мгновенной деавторизацией удалённых), официальные рецептуры, рассылки, журнал безопасности.

Дополнительные защиты ролевой модели: первый зарегистрированный аккаунт автоматически становится руководителем, остальные требуют его подтверждения; смену роли выполняет только руководитель; изменить собственную роль нельзя (защита от self-escalation); актор всегда определяется серверной сессией, а не параметрами запроса.
